...
def lambda_handler(event, context):
    ...
    Supports image compression and format compression 
    """
    ...
    try:
        logger.info(f" Received the event: {json.dumps(event)}")

    # get s3 event details

        for record in event['Records']:
            bucket = record['s3']['bucket']['name']
            key = unquote_plus(record['s3']['object']['key']) #decoding the enconded key
            ...
            #check file_size and log large image warning
            ...
            #upload the images to the processed bucket
            ...
            for processed_image in processed_images:
                ...
                logger.info(f"Uploading the processed image {output_key}")
        ...
        #calculate tottal execution rate
    ...
    #publish custom metrics to CloudWatch
        ...
        publish_metrics(
            funtion_name = context.function_name if context else 'local',
...
def process_image(image_data, original_key,request_id = 'unknown'):
    ...
    try:
        ...
        #convert RGBA to RBG for JPEG compatibility

        if image.mode in ('RGBA','LA','P'):
            #create a white background of size that of image
        ...
        for variant in variants:
            ...
            output_key = f"{base_name}_{variant['sufffix']}_{unique_id}.{extension}"
